module ApplicantsHelper
	...
	def country_code_select(object, method, priority_countries=nil, options={}, html_options={})
	  	...
	  	countries = [Country.all.collect{|r| r.name}, Country.all.collect{|r| r.name}].transpose.sort
	  	...
	  	if priority_countries.nil?
	  		...
	  		output << "<option value=\"\">Country</option>" if options[:include_blank] == true
		    ...
		    unless priority_countries[0].respond_to?(:id)
	  		...
	  		priority_countries_ids.each do |pc|
	  		 	if options[:selected] == pc[:id]
	  				output << "<option value=\"#{pc[:name]}\" selected=\"selected\">#{pc[:name]}</option>"
	  			else
	  				output << "<option value=\"#{pc[:name]}\">#{pc[:name]}</option>"
	  			end	
	  		end	
